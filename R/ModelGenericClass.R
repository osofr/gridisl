...
#' @export
OutcomeModel  <- R6Class(classname = "OutcomeModel",
  cloneable = TRUE,
  portable = TRUE,
  class = TRUE,
  public = list(
    ...
    predict = function(newdata, subset_vars, subset_exprs, ...) {
    ...
    evalMSE = function(testdata) {
      assert_that(self$is.fitted)
      testvals <- testdata$get.outvar(self$subset_idx, self$outvar)
      MSE <- vector(mode = "list", length = ncol(private$probA1))
      names(MSE) <- colnames(private$probA1)
      for (idx in 1:ncol(private$probA1)) {
        MSE[[idx]] <- mean((private$probA1[self$subset_idx, idx] - testvals)^2)
      }
      private$MSE <- MSE
      return(MSE)
    },
    ...
    # take fitted OutcomeModel class object as an input and save the fits to itself
    copy.fit = function(bin.out.model) {
      assert_that("OutcomeModel" %in% class(bin.out.model))
      private$model.fit <- bin.out.model$getfit
      self$is.fitted <- TRUE
      invisible(self)
    },

    # take OutcomeModel class object that contains the predictions for P(A=1|sW) and save these predictions to self$
...
# DeterministicOutcomeModel  <- R6Class(classname = "DeterministicOutcomeModel",
#   inherit = OutcomeModel,
#   cloneable = TRUE,
#   portable = TRUE,
#   class = TRUE,
#   public = list(
