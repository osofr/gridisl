...
make_report_rmd <- function(modelfit, data, K = 5, format = c("html", "pdf", "word"),
                            file.name = getOption('longDiSL.file.name'), file.path = getOption('longDiSL.file.path'),
...
call. = FALSE)
  ...
  report.file <- system.file('report', "report-script-rmd.R", package = 'longDiSL')
  ...
  figure.dir <- file.path(getwd(), "figure/longDiSL-")
